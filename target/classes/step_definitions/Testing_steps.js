addStepDefinitions(function(scenario) {
	// Provide a custom World constructor. It's optional, a default one is
	// supplied.
	scenario.World = function(callback) {
		callback();
	};

	// Define your World, here is where you can add some custom utlity functions
	// you
	// want to use with your Cucumber step definitions, this is usually moved
	// out
	...
	scenario.Given(/^the box in the page is green$/, function(callback) {
		...
		if (boxColor === 'rgb(0, 128, 0)') {
			...
			callback.fail('The box was not green, Expected: ' + boxColor
					+ ' to be "rgb(0, 128, 0)"');
		}
	});
	...
	scenario.Then(/^the box turns red$/, function(callback) {
		...
		if (boxColor === 'rgb(255, 0, 0)') {
			...
			callback.fail('The box was not red, Expected: ' + boxColor
					+ ' to be "rgb(255, 0, 0)"');
		}
	});
